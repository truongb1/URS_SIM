...
local function onPreRender()
  ...
  if veh then
    ...
    local numViews = #M.views
    local windowWidth, windowHeight = 2560, 1440  -- Set the size of the main window

    for i, view in ipairs(M.views) do
      local posX = windowWidth / numViews * (i - 1)
      local posY = 0
      ...
      local window = getOrCreateCameraToWindow(view.name, posX, posY, view.borderless)
      
      -- Calculate the resolution for each view based on the total resolution
      local viewResolutionX = windowWidth / numViews
      resolution.x, resolution.y = viewResolutionX, windowHeight
